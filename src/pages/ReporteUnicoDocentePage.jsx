...
import { FileText, UploadCloud, ListChecks, AlertTriangle, Download, Loader2, XCircle, Trash2, ChevronDown, History } from 'lucide-react';

// --- CONFIGURACIÓN ---
const API_BASE_URL = 'https://kali-ad-web.beesoftware.net/api/reporte-unico-docente';
const BATCH_SIZE = 1000;

// Claves API para el Reporte Único Docente. 
// 'semestre' y 'fechaCarga' se manejan desde la UI, no necesitan estar en la plantilla.
const REPORTE_API_KEYS = [
    'codigoColaborador', 'dni', 'codigoBanner', 'docente',
    'rol2025_1', 'horasPedagogicasM1', 'horasPedagogicasM2', 'alerta',
    'correoDocente', 'sedeDictado', 'facultad', 'carrera', 'responsableProgramacion'
];
...
const ReporteUnicoDocentePage = () => {
    ...
    const [showReport, setShowReport] = useState(true); // Mostrar historial por defecto
    ...
    const handleUploadClick = useCallback(async () => {
        ...
        try {
            // 1. Eliminar datos existentes para el mismo semestre y fecha
            setProcessStatus(`Eliminando datos anteriores para ${selectedSemester} del ${cargaDate}...`);
            ...
            const validPayloads = parsedData.map(({ record: rawRecord }) => {
                ...
                REPORTE_API_KEYS.forEach(apiKey => {
            ...
            // 3. Insertar los nuevos datos en lotes
            const totalRecords = validPayloads.length;
            let successfulInserts = 0;
            for (let i = 0; i < totalRecords; i += BATCH_SIZE) {
                const batch = validPayloads.slice(i, i + BATCH_SIZE);
                setProcessStatus(`Cargando lote ${Math.ceil((i + 1) / BATCH_SIZE)} de ${Math.ceil(totalRecords / BATCH_SIZE)}...`);
            ...
            setUploadSuccess(`Proceso completado. Registros cargados: ${successfulInserts}.`);
            resetUploadState();
            fetchReport(); // Actualizar el historial de cargas
    ...
    const confirmDeleteReport = async () => {
        ...
        setIsProcessing(true);
        setProcessStatus(`Eliminando todos los registros para ${deleteConfirmation.semestre} del ${deleteConfirmation.fechaCarga}...`);
        try {
            const response = await fetch(`${API_BASE_URL}/bulk`, {
                method: 'DELETE',
                headers: { 'Content-Type': 'application/json' },
            ...
            setUploadSuccess(`El lote de carga del ${deleteConfirmation.fechaCarga} para el semestre ${deleteConfirmation.semestre} fue eliminado.`);
            fetchReport(); // Actualizar el historial
        } catch (err) {
    ...
    const handleDownloadTemplate = async () => {
        try {
            const XLSX = await import('https://cdn.sheetjs.com/xlsx-0.20.2/package/xlsx.mjs');
            const templateHeaders = REPORTE_API_KEYS;
            const ws = XLSX.utils.aoa_to_sheet([templateHeaders]);
            const wb = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(wb, ws, "Plantilla");
            XLSX.writeFile(wb, "Plantilla_Reporte_Unico_Docente.xlsx");
        } catch (err) {
        ...
        <div className="container mx-auto p-4 md:p-6 bg-gray-50 min-h-screen">
            <header className="mb-6 md:mb-8">
                <h1 className="text-3xl md:text-4xl font-bold text-gray-800 flex items-center">
                    <FileText className="mr-3 text-teal-600" size={36} /> Gestión de Reporte Único Docente
                </h1>
            </header>
            ...
            {deleteConfirmation && (
                <ConfirmationModal 
                    ...
                    message={`¿Está seguro de que desea eliminar TODOS los registros del semestre ${deleteConfirmation.semestre} para la fecha de carga ${deleteConfirmation.fechaCarga}? Esta acción no se puede deshacer.`}
            ...
            <section className="mb-8 p-4 md:p-6 bg-white rounded-xl shadow-lg">
                <h2 className="text-2xl font-semibold text-gray-700 mb-4 flex items-center">
                    <UploadCloud className="mr-2 text-green-600" /> Carga Masiva de Reportes
                </h2>
                {isProcessing && !deleteConfirmation ? (
                     <div className="my-4">
                        <p className="text-lg text-blue-700 font-medium text-center mb-2">{processStatus}</p>
                        <div className="w-full bg-gray-200 rounded-full h-4"><div className="bg-teal-600 h-4 rounded-full" style={{ width: `${processProgress}%` }}></div></div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-end">
                            ...
                            <div>
                                ...
                                <input type="date" id="carga-date" value={cargaDate} onChange={(e) => setCargaDate(e.target.value)}
                                    className="mt-1 block w-full pl-3 pr-2 py-2 text-base border-gray-300 focus:outline-none focus:ring-teal-500 focus:border-teal-500 sm:text-sm rounded-md"/>
                            </div>
                            <div>
                                ...
                                <input id="file-upload-prog" type="file" ref={fileInputRef} onChange={handleFileSelect}
                                    ...
                                    className="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-teal-50 file:text-teal-700 hover:file:bg-teal-100 disabled:opacity-50 disabled:cursor-not-allowed" />
                            </div>
                        </div>
                            ...
                            <div className="mt-4 p-4 border-t border-gray-200 bg-green-50 rounded-lg">
                                ...
                                <div className="mt-4 flex justify-end">
                                    <button onClick={handleUploadClick}
                                        ...
                                        Actualizar Datos del {cargaDate}
                                    </button>
                                </div>
                            </div>
                        )}
            ...
            <div className="bg-white rounded-xl shadow-lg">
                <button onClick={toggleReport} className="w-full flex justify-between items-center p-4 text-left">
                    <h2 className="text-2xl font-semibold text-gray-700 flex items-center">
                        <History className="mr-2 text-teal-600" /> Historial de Cargas
                    </h2>
                ...
                {showReport && (
                    <div className="p-4 md:p-6 border-t">
                        {isReportLoading ? (
                             <div className="flex items-center justify-center p-5"><Loader2 className="animate-spin h-6 w-6 text-teal-600" /></div>
                        ) : reportError ? (
                                ...
                                {reportData && Object.keys(reportData).length > 0 ? Object.keys(reportData).map(semestre => {
                                    ...
                                    return (
                                        <div key={semestre}>
                                            ...
                                            {mostRecent &&
                                                <div className="relative bg-teal-50 border-l-4 border-teal-500 rounded-r-lg p-4 mb-4 shadow">
                                                    <p className="font-bold text-teal-800">Carga más reciente</p>
                                            ...
                                            {olderReports.length > 0 && (
                                                <div>
                                                    <button onClick={() => toggleSemesterExpansion(semestre)} className="text-sm text-teal-600 hover:text-teal-800 flex items-center mb-2">
...
export default ReporteUnicoDocentePage;
