...
import { Mail, BookOpen, Calendar, Building, TrendingUp, Star, Search, X, Shield, Tag, Briefcase, AlertTriangle, ChevronLeft, ChevronRight, Clock, Loader, History, ChevronDown, Trash2, CheckCircle, UserCheck, MapPin, Edit } from 'lucide-react';
...
const initialApiData = {
    "data": [
        {
            "_id": "685c57b938acc3ced2904a52",
            "idDocente": "N00215514",
            "docente": "ABAD/DEL AGUILA, JULIO CESAR",
            "RolColaborador": "DOCENTE A TIEMPO PARCIAL",
            "programa": "WA",
            "modalidad": "Presencial",
            "promedioEsa": 0.9125,
            "semestre": "2025-1",
            "estadoHistorico": "NUEVO",
            "especialistaDni": "40302370",
            "nombreEspecialista": "VILCA BORCHANI, VICTOR ERMELO",
            "cursos": [
                {
                    "nombreCurso": "NEGOCIOS DIGITALES",
                    "codCurso": "COME1407A",
                    "seccion": "COME.1407A.225515.2028.P",
                    "periodo": "225515",
                    "nrc": "2028",
                    "metEdu": "P",
                    "horarios": [
                        {
                            "fechaInicio": "May 20, 2025 12:00:00 AM",
                            "fechaFin": "Jul 12, 2025 12:00:00 AM",
                            "dia": "MARTES",
                            "hora": "21:10 - 22:40",
                            "campus": "TML",
                            "aula": "C301",
...
// --- NUEVO: Modal para editar especialista ---
const EditSpecialistModal = ({ teacher, specialistOptions, onClose, onSave }) => {
    const [selectedDni, setSelectedDni] = useState(teacher?.especialistaDni || '');

    if (!teacher) return null;

    const handleSave = () => {
        onSave(teacher, selectedDni);
    };

    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 z-[110] flex justify-center items-center p-4">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-lg p-6">
                <h3 className="text-xl font-bold text-gray-800">Editar Especialista</h3>
                <p className="text-gray-600 mt-2">
                    Docente: <span className="font-semibold">{teacher.docente}</span>
                </p>
                <div className="mt-6">
                    <label htmlFor="specialist-select" className="block text-sm font-medium text-gray-700 mb-1">
                    ...
                    >
                        ...
                        {specialistOptions.map(spec => (
                            <option key={spec.dni} value={spec.dni}>{spec.name}</option>
                        ))}
                    </select>
                </div>
                <div className="flex justify-end space-x-4 mt-8">
                    <button onClick={onClose} className="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 border border-gray-300 rounded-lg hover:bg-gray-200">
                        Cancelar
                    </button>
                    <button onClick={handleSave} className="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 flex items-center">
                        Guardar Cambios
                    </button>
                </div>
            </div>
...
export default function AssignmentsPage() {
    ...
    // --- MODIFICADO: Estado para el modal de edición ---
    ...
    const specialistOptions = useMemo(() => {
        ...
        teachers.forEach(t => {
            if (t.especialistaDni && t.nombreEspecialista) {
                specialists.set(t.especialistaDni, t.nombreEspecialista);
            }
        });
        const options = Array.from(specialists, ([dni, name]) => ({ dni, name }));
    ...
    // --- MODIFICADO: Handler para guardar desde el modal ---
    const handleSaveSpecialist = (teacherToUpdate, newSpecialistDni) => {
        // NOTE: This function simulates an API call.
        setTeachers(prevTeachers => {
            return prevTeachers.map(teacher => {
                const currentId = teacher._id?.$oid || teacher.idDocente;
                const updateId = teacherToUpdate._id?.$oid || teacherToUpdate.idDocente;
                if (currentId === updateId) {
                    const selectedSpecialist = specialistOptions.find(s => s.dni === newSpecialistDni);
                    return {
                        ...teacher,
                        nombreEspecialista: selectedSpecialist ? selectedSpecialist.name : null,
                        especialistaDni: selectedSpecialist ? selectedSpecialist.dni : null,
                        estadoHistorico: 'MODIFICADO', // Marcar como modificado
                    };
                }
                return teacher;
            });
        });
        setTeacherToEdit(null); // Cerrar el modal
    };
    ...
    const handleGenerate = async () => {
        ...
        try {
            // Step 1: Limpiar datos del semestre
            setGenerationStatus({
                message: `Paso 1/2: Limpiando datos para el periodo ${periodoAsignacion}...`,
                progress: 10,
                error: false
            });

            const cleanResponse = await fetch('https://kali-ad-web.beesoftware.net/api/sp-da002-limpiar', {
                method: 'POST',
            ...
            if (!cleanResponse.ok) {
                throw new Error(`Error en el paso de limpieza (código: ${cleanResponse.status})`);
            }
            await cleanResponse.json();

            // Step 2: Ejecutar la asignación de especialistas
            setGenerationStatus({
                message: `Paso 2/2: Realizando la asignación de especialistas...`,
                progress: 50,
                error: false
            });

            const assignResponse = await fetch('https://kali-ad-web.beesoftware.net/api/asignacion-especialista-docentes', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ semestre: periodoAsignacion }),
            });
            ...
            if (!assignResponse.ok) {
                let errorMessage = `Error al asignar especialistas (código: ${assignResponse.status})`;
                try {
                    const errorJson = await assignResponse.json();
            ...
            // Step 3: Finalizar y actualizar
            setGenerationStatus({
                message: `Proceso completado. ${assignResult.message || 'Actualizando vista...'}`,
                progress: 90,
                error: false
            });
            await new Promise(res => setTimeout(res, 1500));
            ...
            setGenerationStatus({ message: '¡Asignación y actualización completada con éxito!', progress: 100, error: false });
            ...
            setGenerationStatus({
                ...
                progress: generationStatus.progress, // Keep progress where it failed
                error: true
            });
    ...
    return (
        <div className="bg-slate-100 font-sans min-h-screen p-4 sm:p-6 lg:p-8">
            ...
            />
            {/* --- NUEVO: Renderizar el modal de edición --- */}
            <EditSpecialistModal
                        ...
                        <table className="w-full text-sm text-left text-gray-600">
                            <thead className="text-xs text-gray-700 uppercase bg-gray-50">
                                <tr>
                                    ...
                                    <th scope="col" className="px-6 py-3">Especialista Asignado</th>
                            ...
                            <tbody>
                                {paginatedTeachers.map((teacher) => {
                                    ...
                                    const teacherId = teacher._id?.$oid || teacher.idDocente;
                                    
                                    return (
                                        <tr key={teacherId} className={`border-b hover:bg-gray-50 ${rowStyles.border}`}>
                                            ...
                                            <td className="px-6 py-4">
                                                {teacher.nombreEspecialista || <span className="text-gray-400 italic">No Asignado</span>}
                                            </td>

                                            <td className="px-6 py-4 text-center">
                                                <button onClick={() => setTeacherToEdit(teacher)} className="p-2 text-blue-600 hover:bg-blue-100 rounded-full" title="Editar Especialista">
                                                    <Edit size={16} />
                                                </button>
                                            </td>
