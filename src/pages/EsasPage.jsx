...
import { FileText, UploadCloud, ListChecks, AlertTriangle, Download, Loader2, CalendarClock, XCircle, Search, Trash2, ChevronDown, History } from 'lucide-react';

// --- CONFIGURACIÓN ---
const API_BASE_URL = 'https://kali-ad-web.beesoftware.net/api/esa'; // API endpoint para ESA
const BATCH_SIZE = 1000;

const ESA_API_KEYS = [
    'semestre', 'fechaCarga', 'grupoDocente','tipoDeEncuesta','modalidad','programa','modulo','campus', 'codBanner', 'codPayroll', 'dni', 'nombreDocente',
    'totalMatriculados', 'totalEncuestados', 'porcentajeCobertura',
    'preguntaNps', 'preguntaContribAprendizaje', 'promedioEsa', 'escala',
    'nDetractoresNps', 'nNeutrosNps', 'nPromotoresNps', 'promedioNps', 'ranking'
];
...
const EsasPage = () => {
    ...
    const tableDisplayColumns = useMemo(() => [
        ...
        { header: "Docente", accessor: "nombreDocente" },
        { header: "DNI", accessor: "dni" },
        { header: "Campus", accessor: "campus" },
        { header: "Promedio ESA", accessor: "promedioEsa" },
        { header: "Promedio NPS", accessor: "promedioNps" },
    ...
    const fetchEsas = useCallback(async (semester, date, page = 1) => {
        if (!semester || !date) {
            setRecords([]);
    ...
    const handleSearchClick = () => {
        setCurrentPage(1);
        fetchEsas(filterSemester, filterDate, 1);
    };
    ...
    const handleUploadClick = useCallback(async () => {
        ...
        try {
            setProcessStatus(`Eliminando datos para ${selectedSemester} del ${cargaDate}...`);
    ...
    }, [parsedData, selectedSemester, cargaDate, filterSemester, handleSearchClick, fetchReport]);
    ...
    const handleDownloadTemplate = async () => {
        try {
            const XLSX = await import('https://cdn.sheetjs.com/xlsx-0.20.2/package/xlsx.mjs');
            const templateHeaders = ESA_API_KEYS.filter(k => k !== 'semestre' && k !== 'fechaCarga');
            const ws = XLSX.utils.aoa_to_sheet([templateHeaders]);
            const wb = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(wb, ws, "Plantilla");
            XLSX.writeFile(wb, "Plantilla_Carga_ESA.xlsx");
        } catch (err) {
    ...
    const handlePageChange = (newPage) => {
        if (newPage >= 1 && newPage <= totalPages) {
           fetchEsas(filterSemester, filterDate, newPage);
        }
    };
        ...
        <div className="container mx-auto p-4 md:p-6 bg-gray-50 min-h-screen">
            <header className="mb-6 md:mb-8">
                <h1 className="text-3xl md:text-4xl font-bold text-gray-800 flex items-center">
                    <FileText className="mr-3 text-purple-600" size={36} /> Gestión de Encuestas ESA
                </h1>
            </header>
            ...
            <section className="mb-8 p-4 md:p-6 bg-white rounded-xl shadow-lg">
                ...
                {isProcessing && !deleteConfirmation ? (
                     <div className="my-4">
                        ...
                        <div className="w-full bg-gray-200 rounded-full h-4"><div className="bg-purple-600 h-4 rounded-full" style={{ width: `${processProgress}%` }}></div></div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-end">
                            ...
                            <div>
                                ...
                                <input type="date" id="carga-date" value={cargaDate} onChange={(e) => setCargaDate(e.target.value)}
                                    className="mt-1 block w-full pl-3 pr-2 py-2 text-base border-gray-300 focus:outline-none focus:ring-purple-500 focus:border-purple-500 sm:text-sm rounded-md"/>
                            </div>
                            <div>
                                ...
                                <input id="file-upload-prog" type="file" ref={fileInputRef} onChange={handleFileSelect}
                                    ...
                                    className="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-purple-50 file:text-purple-700 hover:file:bg-purple-100 disabled:opacity-50 disabled:cursor-not-allowed" />
                            </div>
                        </div>
            ...
            <div className="mb-8 bg-white rounded-xl shadow-lg">
                <button onClick={toggleReport} className="w-full flex justify-between items-center p-4 text-left">
                    <h2 className="text-2xl font-semibold text-gray-700 flex items-center">
                        <History className="mr-2 text-purple-600" /> Historial de Cargas
                    </h2>
                ...
                {showReport && (
                    <div className="p-4 md:p-6 border-t">
                        {isReportLoading ? (
                             <div className="flex items-center justify-center p-5"><Loader2 className="animate-spin h-6 w-6 text-purple-600" /></div>
                        ) : reportError ? (
                                ...
                                {reportData && Object.keys(reportData).length > 0 ? Object.keys(reportData).map(semestre => {
                                    ...
                                    return (
                                        <div key={semestre}>
                                            ...
                                            <div className="relative bg-purple-50 border-l-4 border-purple-500 rounded-r-lg p-4 mb-4 shadow">
            ...
            <section className="bg-white rounded-xl shadow-lg p-4 md:p-6">
                 <div className="flex flex-col md:flex-row justify-between items-center mb-4 gap-4">
                    <h2 className="text-2xl font-semibold text-gray-700 flex items-center shrink-0">
                        <ListChecks className="mr-2 text-purple-600" /> Consultar Registros ESA
                    </h2>
                ...
                {isLoading && !isProcessing ? (
                    <div className="flex items-center justify-center p-10"><Loader2 className="animate-spin h-8 w-8 text-purple-600" /><p className="ml-3">Buscando registros...</p></div>
                ) : !hasSearched ? (
